import { extractEntry, parseArchive } from "./archive.js?v=native-glm-2";
...
async function extractAll() {
  if (!("showDirectoryPicker" in window)) return showNotice("Extract all requires Chrome or Edge. You can still extract individual files.");
  const button = $("#extractAll");
  try {
    const root = await window.showDirectoryPicker({ mode: "readwrite" }); button.disabled = true;
    let completed = 0;
    for (const entry of state.entries) {
      if (entry.isDirectory) continue;
      const parts = entry.name.split("/"), filename = parts.pop(); let directory = root;
      for (const part of parts) directory = await directory.getDirectoryHandle(part, { create: true });
      const handle = await directory.getFileHandle(filename, { create: true });
      const writable = await handle.createWritable();
      await writable.write(await extractEntry(state.file, entry)); await writable.close();
      button.textContent = `Extracting ${++completed} / ${state.entries.length}`;
    }
    toast(`Extracted ${completed.toLocaleString()} files with their original paths`);
  } catch (error) { if (error.name !== "AbortError") showNotice(error.message); }
  finally { button.disabled = false; button.textContent = "Extract all"; }
}
